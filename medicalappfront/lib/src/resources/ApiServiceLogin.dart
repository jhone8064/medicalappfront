...
class ApiLogin {
  ...
  Future<UserRegisData> infoUser(String id) async {
    var res = await client
        .get('https://' + Constants.urlAuthority + '/infoUser?id=' + id);
    if (res.statusCode == 200) {
      // If the call to the server was successful, parse the JSON
  ...
  Future<ApiResponse> cancelarCita(UserRegisCita userRegisCita) async {    
    var body = jsonEncode(userRegisCita.toJson2());
    
    Uri uri = Uri.https(Constants.urlAuthority, Constants.urlEditarCita);
    var res = await client.post(uri,
        headers: {HttpHeaders.contentTypeHeader: Constants.contentTypeJson},
        body: body);
    if (res.statusCode == 200) {
